...
```{r}
weekly_stats_spring = weekly_stats(clean_data, tidy_data, start_date_spring, end_date_spring, dining_halls)# |> row_spec(0, background = "#48D1CC", color = "black")
```

```{r}
weekly_stats_fall = weekly_stats(clean_data, tidy_data, start_date_fall, end_date_fall, dining_halls)# |> row_spec(0, background = "#48D1CC", color = "black")
```

```{r}
#| layout-ncol: 2 
#| tbl-cap: "Weekly Statistics"
#| tbl-subcap: ["Spring 2025", "Fall 2025"]
library(knitr)
# Table on the left
kable(weekly_stats_spring) 
# Table on the right
kable(weekly_stats_fall)
```
