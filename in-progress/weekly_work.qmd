...
data <- read_csv("data/FRN_Collection_Form_February_25_2025.csv")
...
work_data_filtered <-  work_data |>
  filter(date >= mdy('02-17-2025')) |>
  filter(date <= mdy('02-23-2025'))

week_weight <- work_data_filtered |>
  summarize(total_weight = sum(weight, na.rm = TRUE))

work_data <- clean_data
work_data$date <- mdy(work_data_cont$date)
work_data$containers <- as.numeric(work_data_cont$containers)
